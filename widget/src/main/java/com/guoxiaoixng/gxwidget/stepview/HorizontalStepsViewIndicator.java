package com.guoxiaoixng.gxwidget.stepview;
...
public class HorizontalStepsViewIndicator extends View {
    ...
    private Drawable mErrorIcon;//正在进行的默认图片     definition default underway icon
    private Drawable mUnCompleteIcon;//默认的背景图  definition default unCompleted icon
    ...
    private int mUnCompletedLineColor = ContextCompat.getColor(getContext(), R.color.fcwidget_fc_c12);//定义默认未完成线的颜色  definition
    private int mCompletedLineColor = ContextCompat.getColor(getContext(), R.color.fcwidget_fc_c12);//定义默认完成线的颜色      definition mCompletedLineColor
    ...
    private void setupView() {
        ...
        mCompleteIcon = ContextCompat.getDrawable(getContext(), R.drawable.fcwidget_complete);//已经完成的icon
        mErrorIcon = ContextCompat.getDrawable(getContext(), R.drawable.fcwidget_error);//正在进行的icon
        mUnCompleteIcon = ContextCompat.getDrawable(getContext(), R.drawable.fcwidget_uncomplete);//未完成的icon
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size(); i++) {
            ...
            if (stepsBean.getState() == StepBean.STEP_UNDO) {
                ...
                mErrorIcon.setBounds(rect);
                mErrorIcon.draw(canvas);
     ...
     * 设置默认图片
     *
     * @param unCompleteIcon unCompleteIcon
     */
    public void setUnCompleteIcon(Drawable unCompleteIcon) {
        this.mUnCompleteIcon = unCompleteIcon;
    }
