...
import com.guoxiaoxing.widget.dialog.widget.picker.model.IDebug;
import com.guoxiaoxing.widget.dialog.widget.picker.model.IPickerModel;
import com.guoxiaoxing.widget.dialog.widget.picker.model.IWheelPicker;
...
public class SCWheelPicker extends View implements IDebug, IWheelPicker, Runnable {
     ...
     * 数据源
     */
    private List<IPickerModel> mData = new ArrayList<IPickerModel>();
    ...
    public SCWheelPicker(Context context, AttributeSet attrs) {
        ...
        mSelectedItemTextColor = a.getColor
                (R.styleable.SCWheelPicker_wheel_selected_item_text_color, ContextCompat.getColor(context, R.color.base_fc_c4));
...
//        computeTextSize();
    ...
    private void computeTextSize() {
        ...
        if (hasSameWidth) {
            mTextMaxWidth = (int) mPaint.measureText(String.valueOf(mData.get(0).getName()));
        } else if (isPosInRang(mTextMaxWidthPosition)) {
            mTextMaxWidth = (int) mPaint.measureText
                    (String.valueOf(mData.get(mTextMaxWidthPosition).getName()));
            ...
            for (IPickerModel pickerModel : mData) {
                String text = pickerModel.getName();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int drawnDataPos = drawnDataStartPos + mSelectedItemPosition,
            ...
            if (isCyclic) {
                ...
                data = String.valueOf(mData.get(actualPos).getName());
            } else {
                if (isPosInRang(drawnDataPos))
                    data = String.valueOf(mData.get(drawnDataPos).getName());
            }
    ...
    @Override
    public List<IPickerModel> getData() {
        return mData;
    }

    @Override
    public void setData(List<IPickerModel> data) {
        if (null == data)
